...
const DEGREE_MATCHERS = [
	{ degree: "B.Tech", pattern: /\bb\.?\s*tech(?:in)?\b/i },
	{ degree: "M.Tech", pattern: /\bm\.?\s*tech(?:in)?\b/i },
...
const extractDurationFromText = (text) => {
	const patterns = [
		new RegExp(
			`(${MONTH_PATTERN}\\s+\\d{4})\\s*[-–]?\\s*(${MONTH_PATTERN}\\s+\\d{4}|present|current|ongoing)`,
			"i"
		),
...
const extractScoreFromText = (text) => {
	let match = text.match(
		/\b(CGPA|SGPA|GPA)\b\s*[:=-]?\s*(\d+(?:\.\d+)?(?:\s*\/\s*\d+(?:\.\d+)?)?)/i
	);

	if (match) {
		return `${match[1].toUpperCase()} ${match[2].replace(/\s+/g, "")}`;
	}
...
export function extractEducation(educationText = "") {
	...
	const lines = educationText
		.split("\n")
		.map((line) => cleanArtifacts(line.trim()))
		.filter(Boolean);
	...
	for (const line of lines) {
		...
		const degree = extractDegreeFromText(line);
		if (degree && !current.degree) {
			current.degree = degree;
		}

		const duration = extractDurationFromText(line);
		if (duration && !current.duration) {
			current.duration = duration;
		}

		const score = extractScoreFromText(line);
	...
	return entries
		.map((entry) => ({
			...
			degree: DEGREE_WHITELIST.includes(entry.degree)
				? entry.degree
				: "",
